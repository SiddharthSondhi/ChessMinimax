...
# Game state checker
class Board:
    def __init__(self, display, width: float, height: float):
    ...
    # return evaluation used for minimax algorithm, negative favors black or positive favors white
    def get_evaluation(self):
        # values added to each piece depending on what square it's on, source: https://www.chessprogramming.org/Simplified_Evaluation_Function
        ...
        eval = 0
        if self.is_in_check("white"): eval -= 100
        elif self.is_in_check("black"): eval += 100
        for piece in [i.occupying_piece for i in self.squares]:
            if piece != None:
                posVal = 0
                ...
                if piece.color == 'white':
                    if piece.notation == 'P':
                        posVal = pawnTable[y][x]
                    elif piece.notation == 'N':
                        posVal = knightTable[y][x]
                    elif piece.notation == 'B':
                        posVal = bishopTable[y][x]
                    elif piece.notation == 'Q':
                        posVal = queenTable[y][x]
                    elif piece.notation == 'R':
                        posVal = rookTable[y][x]
                    elif piece.notation == 'K':
                        posVal = kingTable[y][x]

                    eval += piece.get_value() + posVal

                else:
                    if piece.notation == 'P':
                        posVal = pawnTable[::-1][y][x]
                    elif piece.notation == 'N':
                        posVal = knightTable[::-1][y][x]
                    elif piece.notation == 'B':
                        posVal = bishopTable[::-1][y][x]
                    elif piece.notation == 'Q':
                        posVal = queenTable[::-1][y][x]
                    elif piece.notation == 'R':
                        posVal = rookTable[::-1][y][x]
                    elif piece.notation == 'K':
                        posVal = kingTable[::-1][y][x]
                    eval -= (piece.get_value() + posVal)
        return eval
    ...
    def print_state(self):
        for i in range(len(self.squares)):
            if (self.squares[i].occupying_piece != None):
                print(self.squares[i].occupying_piece.notation, end=' ')
            else:
                print('-', end=' ')
            if i % 8 == 7:
                print()
    ...
    def __deepcopy__(self, memo):
        ...
        new_inst.display = None
        new_inst.width = self.width
        new_inst.height = self.height
        new_inst.tile_width = self.tile_width
        new_inst.tile_height = self.tile_height
        new_inst.selected_square = copy.deepcopy(self.selected_square, memo)
        new_inst.turn = self.turn
        new_inst.config = self.config
        new_inst.squares = copy.deepcopy(self.squares, memo)
